...
# # Write to file without using mpio for h5py #################################
if proc == 0:
    f = h5py.File('svd.h5', 'w')
for i in range(0,nproc):
    # if i == 0, create dataset
    if proc == 0:
        if i == 0:
            dset_sigmaT = f.create_dataset('temperature/sigma', (modes, M), dtype=numpy.float32)
            dset_sigmau1 = f.create_dataset('u1/sigma', (modes, M), dtype=numpy.float32)
            dset_sigmau2 = f.create_dataset('u2/sigma', (modes, M), dtype=numpy.float32)
            dset_UT = f.create_dataset('temperature/U', (M, modes, M), dtype=numpy.float32)
            dset_U1 = f.create_dataset('u1/U', (M, modes, M), dtype=numpy.float32)
            dset_U2 = f.create_dataset('u2/U', (M, modes, M), dtype=numpy.float32)

    # Broadcast arrays from process i to all processes
    STT_temp = comm.bcast(STT, root=i)
    S11_temp = comm.bcast(S11, root=i)
    S22_temp = comm.bcast(S22, root=i)
    UTT_temp = comm.bcast(UTT, root=i)
    U11_temp = comm.bcast(U11, root=i)
    U22_temp = comm.bcast(U22, root=i)

    # Write data
    if proc == 0:
        dset_sigmaT[:,(0 + i*local_size):((i+1)*local_size)] = STT_temp
        dset_sigmau1[:,(0 + i*local_size):((i+1)*local_size)] = S11_temp
        dset_sigmau2[:,(0 + i*local_size):((i+1)*local_size)] = S22_temp
        dset_UT[:,:,(0 + proc*local_size):((proc+1)*local_size)] = UTT_temp
        dset_U1[:,:,(0 + proc*local_size):((proc+1)*local_size)] = U11_temp
        dset_U2[:,:,(0 + proc*local_size):((proc+1)*local_size)] = U22_temp
